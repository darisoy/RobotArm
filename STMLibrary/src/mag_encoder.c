...
void AS5047D_Write(GPIO_TypeDef* CS_GPIO_Port, uint16_t CS_GPIO_Pin, uint16_t address, uint16_t data)
{
	...
	if (HAL_SPI_Transmit(&hspi1, (uint8_t*) &address, 2, 100) != HAL_OK)
	{
		Error_Handler();
	}
...
uint16_t AS5047D_Read(GPIO_TypeDef* CS_GPIO_Port, uint16_t CS_GPIO_Pin, uint16_t address)
{
	...
	if (HAL_SPI_Transmit(&hspi1, (uint8_t*) &address, 1, 100) != HAL_OK)
	{
		Error_Handler();
	}
	...
	if (HAL_SPI_Receive(&hspi1, (uint8_t*) &data, 1, 100) != HAL_OK)
	{
		Error_Handler();
	}
...
{
	/* Initiaize AS4047D */
	AS5047D_Write(AS4047D_CS1_Port, AS4047D_CS1_Pin , AS4047D_SETTINGS1, 0x5);

	AS5047D_Write(AS4047D_CS1_Port, AS4047D_CS1_Pin , AS4047D_SETTINGS2, 0x0);
